...
struct SubstringArray* lines(Substring haystack) {
  ...
  while (last_line_so_far.end < haystack.end) {
    if (*last_line_so_far.end == '\n') {
      output = append_to_substring_array(output, last_line_so_far);   //append substring up to '\n' to output array
	  last_line_so_far.start = last_line_so_far.end + 1;			  //update start pointer to character after '\n'
    }
